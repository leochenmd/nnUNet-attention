...
class nnUNetLoggerClassify(object):
    """
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'val_losses':
            new_ema_val_loss = self.my_fantastic_logging['ema_val_loss'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['ema_val_loss']) > 0 else value
            self.log('ema_val_loss', new_ema_val_loss, epoch)


        #csv_df = pd.DataFrame.from_dict(self.my_fantastic_logging, orient='index')
        #csv_filepath = join(output_folder, "training_curve.csv")
