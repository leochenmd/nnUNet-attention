...
from nnunetv2.training.logging.nnunet_logger_classify import nnUNetLoggerClassify
...
class ClassifyinnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if '2d' in self.configuration_name:       #2D model
            self.segmentation_model_path = os.path.join(r'C:\Users\Leo\Documents\UHN-MedImg3D-ML-quiz\nnUNet_results\Dataset001_Pancreas\nnUNetTrainer__nnUNetPlans__2d\fold_' + str(fold), 'checkpoint_best.pth')
        elif '3d' in self.configuration_name:     #3D model
            self.segmentation_model_path = os.path.join(r'C:\Users\Leo\Documents\UHN-MedImg3D-ML-quiz\nnUNet_results\Dataset001_Pancreas\nnUNetTrainer__nnUNetPlans__3d_fullres\fold_' + str(fold), 'checkpoint_best.pth')
        ...
        self.num_iterations_per_epoch = 201
        self.num_val_iterations_per_epoch = 51
        self.num_epochs = 50
        self.current_epoch = 0
        self.enable_deep_supervision = False    ### doesn't matter, because there is only one classification output in this head
        ...
        self.log_file = join(self.output_folder, "training_log_%d_%d_%d_%02.0d_%02.0d_%02.0d.txt" %
                             (timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute,
                              timestamp.second))
        self.logger = nnUNetLoggerClassify()
    ...
    def _build_loss(self, include_segmentation=False):
        if include_segmentation == True:
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_tr = nnUNetDatasetClassify(self.preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
        dataset_val = nnUNetDatasetClassify(self.preprocessed_dataset_folder, val_keys,
                                    folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                    num_images_properties_loading_threshold=0)
    ...
    def get_dataloaders(self):
        ...
        ### need to modify data loaders so that the lesion class is included
        if dim == 2:
            dl_tr = nnUNetDataLoader2Dclassify(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader2Dclassify(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            dl_tr = nnUNetDataLoader3Dclassify(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader3Dclassify(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
...
########################################################################################
        ...
        # update the model
        self.network.load_state_dict(classify_model_weights)


        # freeze the pretrained weights
        #for name, param in self.network.named_parameters():
        #    if 'convs' in name:                                 # schema 1: Freeze all of the pretrained weights (convs + classify convs)
        #        param.requires_grad = False

            #if 'convs' in name and 'classify' not in name:     # schema 2: Freeze all of the pretrained weights (convs only)
            #    param.requires_grad = False
...
########################################################################################
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']        # segmentation ground truth
        lesion_class = batch['lesion_class']
        ...
        if isinstance(lesion_class, list):
            lesion_class = [i.to(self.device, non_blocking=True) for i in lesion_class]
        else:
            lesion_class = lesion_class.to(self.device, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss(output, lesion_class)

            #######
            ### need to update the loss function as lesion loss
            #######
    ...
    def validation_step(self, batch: dict) -> dict:
        # need to modify this also, because we don't need to predict the segmentation maps
    ...
    def on_epoch_end(self):
        ...
        if self._best_ema is None or self.logger.my_fantastic_logging['ema_val_loss'][-1] < self._best_ema:
            self._best_ema = self.logger.my_fantastic_logging['ema_val_loss'][-1]
