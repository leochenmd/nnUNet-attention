...
class nnUNetDataLoader3Dclassify(nnUNetDataLoaderBaseClassify):
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):     # 'i' is the key (quiz_2_413) and 'j' is the index 0-2
            ...
            data, seg, properties, lesion = self._data.load_case(i)
            ...
            #lesion_all[j, int(lesion)] = 1              # convert to one hot encoding of lesion class
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    lesion_all = torch.from_numpy(lesion_all).type(torch.LongTensor) #float()
